...
const loadImages = (function () {
    ...
    //打开红包后金币及红包掉落动画
    for (let i = 1; i < 55; i++) {
        redGold.push(`./img/StartView/Popup_red/Popup_red_gold/redpack_gold_${i}.png`);
    }
    return {
        imgBg,
        redBoxList,
...
function openRedBox() {   
    ...
    TMIER.timerGold = setTimeout(function () {
        ani.GoldAnimation = animation().loadImage(loadImages.redGold).changeSrc(openRedGold, loadImages.redGold).start(40);
    }, 500);
...
function closeBox() {
    ...
    smallRed.classList.remove('hidden');
    redTitle.classList.remove('hidden');
    redBoxClick.classList.remove('hidden');

    //清除跳转定时器
    clearTimeout(TMIER.timerRun);
    //清除金币动画定时器
    clearTimeout(TMIER.timerGold);
    // 释放动画资源
    disposeAni();
}
...
function triggerEvent() {
    ...
    //点击弹出红包窗口
    smallRed.addEventListener('click', popBox, false);
    //点击红包打开
